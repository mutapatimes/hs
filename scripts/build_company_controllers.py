...
def build(psc: list[Path], companies: Path, out: Path, replace: bool, limit: int) -> None:
    ...
    # active + non-shell test and tier by size/industry.
    ...
    for row in _open_company_csv(companies):
        ...
        for display, surname in candidates[number]:
            ...
            if norm:
                rows.setdefault(norm, (display, tier, company_display, industry))
                kept += 1
    ...
    with out.open("w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        fh.write("# UK company controllers (eponymous owners, 75%+ control) — Companies House, OGL v3.0.\n")
